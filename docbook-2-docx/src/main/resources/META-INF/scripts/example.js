...
function exampleTable(content) {
    const border = WmlAdapter.getBorder(STBorder.SINGLE, 4, 0, "E0E0DC");
    const tblBorders = WmlBuilderFactory.getTblBordersBuilder().withTop(border).withLeft(border)
        .withBottom(border).withRight(border).withInsideH(border).withInsideV(border)
        .getObject();
    const tblPr = WmlBuilderFactory.getTblPrBuilder().withTblBorders(tblBorders).getObject();
    // var shade = WmlBuilderFactory.getCTShdBuilder().withVal(STShd.CLEAR).withColor("auto")withFill("FFFEF7").getObject();
    // var tcPr = WmlBuilderFactory.getTcPrBuilder().withShd(shade).getObject();

    const shade = WmlBuilderFactory.getCTShdBuilder().withVal(STShd.CLEAR).withColor("auto").withFill("FFFFE0").getObject();
