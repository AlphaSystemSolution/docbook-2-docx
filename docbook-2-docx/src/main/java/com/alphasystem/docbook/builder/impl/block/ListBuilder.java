...
public abstract class ListBuilder<S> extends AbstractBuilder<S> {
  ...
  protected void preProcess() {
    ...
    final var variableListParents =
        allParents.stream().filter(ListBuilder::isVariableListBuilder).toList();
    final var otherListBuilders =
        allParents.stream().filter(ListBuilder::isOtherListBuilder).toList();
    final var noListBuilders = variableListParents.isEmpty() && otherListBuilders.isEmpty();
    final var nestedVariableList =
        AppUtil.isInstanceOf(VariableListBuilder.class, this) && variableListParents.size() == 1;
