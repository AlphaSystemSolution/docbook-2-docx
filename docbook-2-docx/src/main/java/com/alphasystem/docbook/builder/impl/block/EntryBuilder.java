...
public class EntryBuilder extends AbstractBuilder<Entry> {
  ...
  protected void preProcess() {
    ...
    if (!AppUtil.isInstanceOf(TableContentBuilder.class, contentBuilder)) {
      // EntryBuilder immediate parent should be RowBuilder and RowBuilder parent must be either of
      // TableBodyBuilder,
      // TableHeaderBuilder, or TableFooterBuilder, if not raise exception
      throw new RuntimeException(
          String.format("Found different parent \"%s\".", contentBuilder.getClass().getName()));
    }

    final var parentVAlign =
        (VerticalAlign) AppUtil.invokeMethod(contentBuilder.getSource(), "getVAlign");
          ...
          .map(
              content -> {
                ...
                if (UnmarshallerConstants.isParaTypes(content)) {
                  ...
                  final var ppr =
                      new PPrBuilder(
                              WmlBuilderFactory.getPPrBuilder().withJc(align).getObject(),
                              p.getPPr())
                          .getObject();
                  p.setPPr(ppr);
  ...
  protected List<Object> doProcess(List<Object> processedChildContent) {
    ...
    final var columnData =
        new ColumnData(columnIndex)
            .withColumnProperties(tcPr)
            .withGridSpanValue(gridSpan)
            .withVerticalMergeType(vMergeType)
            .withContent(processedChildContent.toArray());
    final var tc =
        TableAdapter.createColumn(
            tableBuilder.getTableType(),
            tableBuilder.getTableFormat(),
            tableBuilder.isKeepTableTogether(),
            columnData,
            tableBuilder.getColumnInfoList());
    return Collections.singletonList(tc);
  }

  private static STVerticalJc getVerticalAlign(
      BasicVerticalAlign vAlign, VerticalAlign parentVerticalAlign) {
    if (vAlign == null && parentVerticalAlign != null) {
      try {
        vAlign = BasicVerticalAlign.fromValue(parentVerticalAlign.value());
      } catch (Exception e) {
        // ignore, if the parent value is BASELINE, then there is no corresponding value for
        // BasicVerticalAlign
      }
    }
