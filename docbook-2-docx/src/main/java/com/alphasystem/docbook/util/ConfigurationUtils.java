...
public class ConfigurationUtils {
    ...
    private void loadAdmonitions() {
        ...
        Arrays.stream(Admonition.values())
                .forEach(
                        admonition -> {
    ...
    public String[] getStyles() throws SystemException {
        ...
        if (customDirPath != null) {
            final var styles = String.format("%s%s%s", customDirPath, File.separator, "styles");
            final var consumer = (Function<Path, String>) path -> path.toAbsolutePath().toString();
            final var customStyles = AppUtil.processDirectory(Paths.get(styles), consumer);
            customStyles.addFirst(defaultStyles);
            return customStyles.toArray(new String[0]);
        }
        return new String[] {defaultStyles};
    }
