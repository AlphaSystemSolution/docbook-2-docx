...
public abstract class HandlerFactory<H> {
  ...
  public void registerHandler(String key, H handler) {
    ...
    // service handler loads service from the resources of the client jar then it comes to
    // docbook-2-docx
    // we would like to honour handler(s) from the client first, so that client can override default
    // handler(s)
